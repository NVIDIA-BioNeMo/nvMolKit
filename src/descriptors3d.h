...
inline constexpr std::array<Property3D, 4> kAllProperty3D = {
  Property3D::PMI1,
  Property3D::PMI2,
  Property3D::PMI3,
 ...
 * are converted on load. Atom weights are stored once per molecule in CSR form
 * (@p moleculeAtomStarts, length `coordinates.nMols + 1`) and resolved per conformer through
 * `coordinates.molIndices`. Conformers whose molecule index is out of range, whose atom range lies
